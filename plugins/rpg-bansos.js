...
let handler = async (m, { conn, args, usedPrefix, DevMode }) => {
  try {
    ...
    if (new Date - user.lastbansos > 300000) {
      if (Aku > Kamu) {
        conn.sendFile(m.chat, kbansos, 'korupsi.jpg', `Kamu Tertangkap Setelah Kamu korupsi dana bansos🕴️💰,  Dan kamu harus membayar denda 3 Juta rupiah💵`, m)
        user.money -= 3000000
        user.lastbansos = new Date * 1
      } else if (Aku < Kamu) {
        user.money += 3000000
        conn.sendFile(m.chat, mbansos, 'korupsi.jpg', `Kamu berhasil  korupsi dana bansos🕴️💰,  Dan kamu mendapatkan 3 Juta rupiah💵`, m)
        user.lastbansos = new Date * 1
      } else {
        m.reply(`Sorry Gan Lu g Berhasil Korupsi bansos Dan Tidak masuk penjara karna kamu *melarikan diri🏃* ${botol}`)
        user.lastbansos = new Date * 1
      }
